(function (Raphael) {
  ...
  {
    return new Planarity(this, n, cx, cy);
  }
  ...
  var Planarity = function (paper, n, cx, cy)
  {
    ...
    // check if the level is clear once per second
    var that = this;
    setInterval(function () {that.checkLevelClear(vertices[0])}, 1000);
  }
  
  Planarity.prototype.checkLevelClear = function (obj) {
    if (!this.cbLevelClear) return;
    var anyIntersections = 0;
    obj.graph.intersecting(function () {
      anyIntersections = 1;
    });
    if (!anyIntersections) {
      this.cbLevelClear();
      delete this.cbLevelClear; // only call it once
    }
  }
  
  Planarity.prototype.onLevelClear = function (cb) {
    this.cbLevelClear = cb;
  }
})(Raphael);
